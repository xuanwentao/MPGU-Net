...
selected_dataset = "PPNMdata50"
run_all = False  # True: 跑所有数据集，False: 跑 selected_dataset

dataset_config = {
    "PPNMdata50": {
        "data_path": "PPNMdata50.mat",
        "save_path": "./result/resultourPPNM_PPNMdata50.mat",
        "lamba_re": 1,
        "lamba_lin_non": 1,
        "lamba_lin": 0,
        "lamba_vol":1e-4,
...
def train(model, dataloader, epochs, lamba_re,lamba_lin_non,lamba_lin, lamba_vol, lamba_sad,lamba_kl):
    ...
    for epoch in range(epochs):
        ...
        for x1 in dataloader:
            ...
            safe_para = parameter.squeeze(0)  # shape: [1, H*W]

            expr = 1 + 4 * safe_para * x1
            expr_clamped = torch.clamp(expr, min=1e-6)  

            denom = 2 * safe_para
            denom = denom + 1e-6

            linear_cap = (-1 + torch.sqrt(expr_clamped)) / denom
...
if __name__ == "__main__":
    ...
    for dataset_name in target_datasets:
        ...
        model = PPNMModel(Nb, EndNum,config["z_dim"]).to(device)
